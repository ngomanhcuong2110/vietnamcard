...
def get_each_number(img, normal_img):
    ...
    if not text:
        text = pytesseract.image_to_string(
            normal_img, lang='vie', config='--psm 6')
        if not text:
            text = '?'
    return text
...
def get_text(img):
    ...
    text = pytesseract.image_to_string(Image.open(
        filename), lang='vie', config='--psm 6')
    if not text:
...
def get_id_numbers_text(img):
    ...
    img = cv2.resize(
            img, (int(width_img/2), int(height_img/2)), interpolation=cv2.INTER_CUBIC)
    img_gray=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    img_gray=img_gray[:,int(img_gray.shape[1]/5):int(img_gray.shape[1]*6/7)]
    img_bitnot=cv2.bitwise_not(img_gray)
    cv2.imwrite("id.jpg", img_bitnot)
    id=pytesseract.image_to_string(img_bitnot, lang='eng',config='--psm 6')
    print("iD: ",id)
    height_img, width_img, _ = img.shape
    if height_img < 20:
        img = cv2.resize(
